create or replace PROCEDURE generate_eod_sub_sap_report
(
...
AS 
    ...
    IS
    SELECT DISTINCT DECODE(c_aggregate_flag, 'CR-AGG', SUB_SAP_CR,'BOTH-AGG',SUB_SAP_CR, SUB_SAP_DR) ACCOUNTS
    FROM msgdb
    WHERE msgdb_id IN 
                    (
                        SELECT msgdb_id FROM msgdb_output
    ...
    AND SUB_SAP_CR IS NOT NULL
    AND SUB_SAP_DR IS NOT NULL
    AND COMPANY_CODE = p_company_code
    ...
    IS
    SELECT DISTINCT DECODE(c_aggregate_flag, 'CR-AGG', encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_CR),'BOTH-AGG',encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_CR), encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_DR)) ACCOUNTS
    FROM msgdb
    WHERE msgdb_id IN
                    (
                        SELECT msgdb_id FROM msgdb_output
    ...
    AND aggregate_flag IN (SELECT para_code FROM TABLE(get_code_from_list (c_aggregate_flag||','||'BOTH-AGG,IND', ',')))
    AND groupinginfo_eod is not null
    AND NVL(sub_sap_cr, 'XXXX') = DECODE(c_aggregate_flag, 'CR-AGG',c_distinct_accounts, NVL(sub_sap_cr, 'XXXX'))
    AND NVL(sub_sap_dr, 'XXXX') = DECODE(c_aggregate_flag, 'DR-AGG',c_distinct_accounts, NVL(sub_sap_dr, 'XXXX'))
    ...
    AND aggregate_flag IN (SELECT para_code FROM TABLE(get_code_from_list (c_aggregate_flag||','||'BOTH-AGG,IND', ',')))
    AND groupinginfo_eod is not null
    AND NVL(encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_CR), 'XXXX') = DECODE(c_aggregate_flag, 'CR-AGG',c_distinct_accounts_enc, NVL(encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_CR), 'XXXX'))
    AND NVL(encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_DR), 'XXXX') = DECODE(c_aggregate_flag, 'DR-AGG',c_distinct_accounts_enc, NVL(encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,SUB_SAP_DR), 'XXXX'))
    ORDER BY DECODE(c_aggregate_flag, 'CR-AGG',sap_cr,sap_dr) DESC;
    ...
    AND NVL(sub_sap_cr, 'XXXX') = DECODE(c_aggregate_flag, 'CR-AGG',c_distinct_accounts, NVL(sub_sap_cr, 'XXXX'))
    AND NVL(sub_sap_dr, 'XXXX') = DECODE(c_aggregate_flag, 'DR-AGG',c_distinct_accounts, NVL(sub_sap_dr, 'XXXX'))
    ORDER BY DECODE(c_aggregate_flag, 'CR-AGG',sub_sap_cr,sub_sap_dr) DESC;
...
BEGIN
    ...
    --DBMS_OUTPUT.PUT_LINE('p_company_code : '||p_company_code);
...
--                END IF;
                ...
                THEN
                        ...
                        BEGIN
                            ...
                            THEN
                                ...
                                THEN
                                    ...
                                    DBMS_LOB.writeappend(l_new_file,LENGTH(m_document_header),m_document_header);
                                END IF;
                                --p_file_tran_count   := a_msgblocks.COUNT;
                                ...
                                LOOP
                                    ...
                                    THEN
                                                IF INSTR(m_tenant_list,m_tenantname) > 0
                                                THEN
                                                m_compare_account := encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_cr);
                                                ELSE
                                                m_compare_account := a_msgblocks(rec).sub_sap_cr;
                                                END IF;
                                    
                                    ELSE
                                                IF INSTR(m_tenant_list,m_tenantname) > 0
                                                THEN
                                                m_compare_account := encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_dr);
                                                ELSE
                                                m_compare_account := a_msgblocks(rec).sub_sap_dr;
                                                END IF;
                                    END IF;
                                    ...
                                    THEN
                                        ...
                                        THEN
                                                   IF INSTR(m_tenant_list,m_tenantname) > 0
                                                   THEN
                                                      m_aggregation_account := encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_cr);
                                                   ELSE
                                                    m_aggregation_account := a_msgblocks(rec).sub_sap_cr;
                                                   END IF;
                                        ELSE
                                                   IF INSTR(m_tenant_list,m_tenantname) > 0
                                                   THEN
                                                     m_aggregation_account := encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_dr);
                                                   ELSE
                                                      m_aggregation_account :=a_msgblocks(rec).sub_sap_dr;
                                                   END IF;
                                    ...
                                    END IF;
                                    IF  m_aggregation_type = 'CR-AGG' and (m_aggregation_account =  encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_cr) OR m_aggregation_account = a_msgblocks(rec).sub_sap_cr) and a_msgblocks(rec).msgblocktype = td_get_value(p_mode,'BLOB|CR-AGG')
                                    THEN
                                        m_txn_sum           := m_txn_sum        + NVL(a_msgblocks(rec).priorityamountnum,0);
                                    ELSIF m_aggregation_type = 'DR-AGG' and (m_aggregation_account =  encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_dr) OR m_aggregation_account =a_msgblocks(rec).sub_sap_dr )and a_msgblocks(rec).msgblocktype = td_get_value(p_mode,'BLOB|DR-AGG')
                                    THEN
                                        m_debit_txn_sum     := m_debit_txn_sum  + NVL(a_msgblocks(rec).priorityamountnum,0);
                                    END IF;

                                    IF m_aggregation_type = 'CR-AGG' and (m_aggregation_account =  encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_cr) OR m_aggregation_account =a_msgblocks(rec).sub_sap_cr) and a_msgblocks(rec).msgblocktype = td_get_value(p_mode,'BLOB|CR-AGG|CHARGES')
                                    THEN
                                        DBMS_OUTPUT.PUT_LINE('check1 : ');
                                        m_credit_charges_sum       := m_credit_charges_sum    + NVL(a_msgblocks(rec).MDBPAY_CHARGE_1,0);
                                    ELSIF m_aggregation_type = 'DR-AGG' and (m_aggregation_account =  encrypt_decrypt_basedon_session_cntx('DECRYPT',p_institutionid,a_msgblocks(rec).sub_sap_dr)OR m_aggregation_account =a_msgblocks(rec).sub_sap_dr) and a_msgblocks(rec).msgblocktype = td_get_value(p_mode,'BLOB|DR-AGG|CHARGES')
                                    THEN
					...
					--picking up file from specific blocktype in msgblocks --TJ>>to add blob for charges in configuration
                                                END IF;
                                    ...
                                    THEN
                                        m_txn_sum           := m_txn_sum        + NVL(a_msgblocks(rec).priorityamountnum,0);
                                                   ...
                                                   ELSE
                                                      m_append_message := blob_to_clob(a_msgblocks(rec).MSGDB_ID,td_get_value(p_mode,'BLOB|CR-AGG'));         --picking up file from specific blocktype in msgblocks
                                                   END IF;
                                    ...
                                    THEN
                                        ...
                                        --add both blob for IND
                                                 IF INSTR(m_tenant_list,m_tenantname) > 0
                                                 THEN
                                                      m_append_message := encrypt_decrypt_basedon_session_cntx_clob('DECRYPT',p_institutionid,blob_to_clob(a_msgblocks(rec).MSGDB_ID,td_get_value(p_mode,'BLOB|'||m_aggregation_type)));
                                                 ELSE
                                                      m_append_message := blob_to_clob(a_msgblocks(rec).MSGDB_ID,td_get_value(p_mode,'BLOB|'||m_aggregation_type));         --picking up file from specific blocktype in msgblocks
                                                 END IF;
                        ...
                        THEN
                            genaudit_insert_enchash_wrap
                                (null,null,NULL,'EVNTSRVR','FILE','INSERT','GENERATE_EOD_SUB_SAP_REPORT Procedure Error : OTHERS1...'||SQLCODE || SQLERRM || dbms_utility.format_error_backtrace
                                || SQLCODE
                                || ':'
                                || SQLERRM
    ...
    THEN
        ...
        DBMS_LOB.writeappend(l_new_file,2,CHR(13)||CHR(10));    
        --DBMS_OUTPUT.PUT_LINE ('m_total_debit_sum' || m_total_debit_sum);
        --DBMS_OUTPUT.PUT_LINE ('m_total_credit_sum' || m_total_credit_sum );
            
        m_total_priorityamtnum   := m_total_credit_sum + m_total_debit_sum;
        
        
        --DBMS_OUTPUT.PUT_LINE('m_total_priorityamtnum  : ' || m_total_priorityamtnum );
...
THEN
    ROLLBACK;
    genaudit_insert_enchash_wrap
        (null,null,NULL,'EVNTSRVR','FILE','INSERT','GENERATE_EOD_SUB_SAP_REPORT Procedure Error : OTHERS2...'||SQLCODE || SQLERRM || dbms_utility.format_error_backtrace
        || SQLCODE
        || ':'
        || SQLERRM
        || dbms_utility.format_error_backtrace,
