...
AS
    ...
    (
        ...
        m_institutionid ACCOUNT_MASTER.INSTITUTION_ID%type
    )
    IS
    SELECT   ab.*
    ...
    AND      (ab.ACCOUNT_NUMBER_ENC = encrypt_decrypt_basedon_session_cntx(m_encrypt,m_institutionid,m_account_number,p_data_keyid_list)
    OR        ab.ACCOUNT_NUMBER = m_account_number)
    AND      ab.INSTITUTION_ID  = m_institutionid
    ...
    m_acc_no                            ACCOUNT_MASTER.ACCOUNT_NUMBER%TYPE                              := NULL;
    ...
    m_datakeyid                         VARCHAR2(32664)                                                    := NULL;
...
BEGIN
    ...
    m_datakeyid         := p_data_keyid_list;

    FOR acc_rec IN 1..length(p_account_number) - length(replace(p_account_number,',',null))+1
    LOOP
        m_acc_no    := getstringitemwithsep(p_account_number,acc_rec,',');
        m_institution_id    := getstringitemwithsep(p_institution_id,acc_rec,',');
        m_accounts          := encrypt_decrypt_basedon_session_cntx(m_decrypt,m_institution_id,m_acc_no,m_datakeyid);
        ...
        BEGIN
            ...
            WHERE   institutionid=m_institution_id
        ...
        THEN
            ...
            THEN
                SELECT LISTAGG(encrypt_decrypt_basedon_session_cntx(m_decrypt,m_institution_id,ACCOUNT_NUMBER_ENC,m_datakeyid),',')
                WITHIN GROUP( ORDER BY ACCOUNT_NUMBER_ENC)
                INTO    m_accounts
                FROM    ACCOUNT_MASTER
                WHERE   institution_id = m_institution_id
        ...
        LOOP
            ...
            BEGIN
                ...
                WHERE   (ACCOUNT_NUMBER_ENC = encrypt_decrypt_basedon_session_cntx(m_encrypt,m_institution_id,m_account_number,m_datakeyid)
                OR       ACCOUNT_NUMBER     = m_account_number
                )
                AND     INSTITUTION_ID  = m_institution_id
            ...
            THEN
                ...
                LOOP
                    ...
                    THEN
                        out_rec.account_number  := return_masked_info('PREFIX',m_institution_id,m_account_number);
            ...
            OPEN c_accountbalance_recs(m_account_number,m_institution_id);
                ...
                THEN
                    ...
                    LOOP
                        ...
                        THEN
                            ...
                            out_rec.account_number  := encrypt_decrypt_basedon_session_cntx(m_encrypt,m_institution_id,m_account_number,m_datakeyid);
                            out_rec.entry_date      := m_start_date;
                            out_rec.BALANCES        := 0;
                            out_rec.BALANCE_TYPE    := 'A';
                            PIPE ROW(out_rec);
                            ...
                            ELSE
                                ...
                                THEN
                                    ...
                                    BEGIN
                                        ...
                                        AND     (ACCOUNT_NUMBER_ENC = encrypt_decrypt_basedon_session_cntx(m_encrypt,m_institution_id,m_account_number,m_datakeyid)
                                        OR      ACCOUNT_NUMBER     = m_account_number
